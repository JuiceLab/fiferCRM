/// <reference path="../../Areas/Dashboard/Views/_ViewStart.cshtml" />
$(document).ready(function () {
	elFinder.prototype.commands.editimage = function () {
		var self = this,
		fm = this.fm,
		mimes = fm.res('mimes', 'image') || [],
		 ...
		 **/
		edit = function (id) {
		    loadModalContent('myModal', 'imageedit', 'dashboard/image', '?imageId=' + id );
		...
		this.exec = function (hashes) {
			var img = this.files(hashes)[0];
			var isExist = false;
			$.when($.get("/dashboard/image/checkexist?imagePath=" + img.name).then(function (result) {
				if (result.id == 0) {
					return $.Deferred().reject();
				}
				edit(result.id);
				return img.length
                    ? $.when.apply(null, this.files(hashes))
                    : $.Deferred().reject();
			}));
		...
		filter = function (files) {
			return $.map(files, function (file) {
				return (file.mime.indexOf('image/') === 0 || $.inArray(file.mime, mimes) !== -1)
	...
	elFinder.prototype.i18.ru.messages['cmdeditimage'] = 'Р РµРґР°РєС‚РёСЂРѕРІР°С‚СЊ';
